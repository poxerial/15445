sqlite> WITH decades_bias (title_id, premiered, rating) AS (
(x1...> SELECT titles.title_id, premiered / 10 * 10, rating 
(x1...> FROM titles, ratings
(x1...> WHERE titles.title_id == ratings.title_id
(x1...> AND premiered NOT NULL
   ...> )
   ...> SELECT premiered || 's', ROUND(AVG(rating), 2) AS avg_rating, 
   ...> MAX(rating), MIN(rating), COUNT(*)
   ...> FROM decades_bias
   ...> GROUP BY premiered
   ...> ORDER BY avg_rating DESC, premiered ASC;
